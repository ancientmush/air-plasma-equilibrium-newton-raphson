#+TITLE: Numerical Analysis on Equilibrium Composition of Air Plasma
#+AUTHOR: Wataru Furutaka
#+DESCRIPTION: A Fortran-based program to calculate the chemical equilibrium composition of high-temperature air plasma using Gibbs free energy minimization and a damped Newton-Raphson method with backtracking line search.
#+OPTIONS: toc:t num:t \n:t
...
Where $a_1 \dots a_7$ are the polynomial coefficients, and $b_1, b_2$ are integration constants. These parameters are stored in [[file:src/mod_constants.f90][src/mod_constants.f90]].
